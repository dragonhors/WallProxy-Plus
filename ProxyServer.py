...
import struct, socket, select, threading
from util import urlinfo
import wpconfig

class ProxyServer(SocketServer.ThreadingTCPServer):
    allow_reuse_address = HTTPServer.allow_reuse_address
    daemon_threads = True

    def __init__(self, config, RequestHandlerClass):
        if 'debuglevel' in config:
            from util.urlfetch import HTTPFetch
            HTTPFetch.debuglevel = config['debuglevel']
...
class ProxyRequestHandler(BaseHTTPRequestHandler):
    ...
    def log_request(self, code='-', size='-'):
        self.log_message('"%s" %s %s %s', self.requestline, code, size, self.request_count)
    ...
    def parse_request(self):
        ...
        if not self.server.keep_alive:
            ...
            pcon = 'connection' if self.path[0]=='/' else 'proxy-connection'
            pcon = self.headers.get(pcon, '').lower()
            if 'close' in pcon:
                self.close_connection = 1
    ...
    def handle_https_request(self):
        ...
        if not wpconfig.check_client(self.client_address[0], self.request,
            ...
            return wpconfig.request_auth(self)
    ...
    def handle_http_request(self):
        ...
        if self.path[0] == '/':
            ...
            self.requestline = '%s %s %s' % (self.command, self.url, self.request_version)
        else:
            self.url = self.path
